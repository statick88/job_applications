...
async def login_and_get_cookies(page):
    """Login to Encuentra Empleo and extract fresh cookies."""
    print("Attempting auto-login...")
    
    # Get credentials from environment variables
    username = os.environ.get("ENCUENTRA_EMPLEO_USER")
    password = os.environ.get("ENCUENTRA_EMPLEO_PASS")
    
    if not username or not password:
        print("  No credentials in environment variables (ENCUENTRA_EMPLEO_USER/PASS)")
        return False
    
    try:
        # Go to logout first to ensure clean state
        ...
        # Click the aspirante button to load the login form
        ...
        # Wait for Turnstile CAPTCHA to complete (up to 30 seconds)
        ...
        if not turnstile_completed:
            print("  ⚠️ Turnstile not completed within 30s, trying anyway...")
        
        # Click login button using JavaScript (bypasses visibility check)
        ...
        # Check if login succeeded (should redirect to aspirante area)
        current_url = page.url
        print(f"  Post-login URL: {current_url}")
        
        # Try to go to busquedaOferta to verify session
        await page.goto(URL, timeout=40000)
        await page.wait_for_timeout(3000)
        
        if "busquedaOferta.jsf" in page.url:
            print("  ✅ Login successful - session validated")
            ...
            if jsessionid:
                ...
                print("  ⚠️ Login appeared successful but JSESSIONID not found in cookies")
                return False
        else:
            print(f"  ❌ Login failed - redirected to: {page.url}")
            return False
...
async def apply_to_jobs():
    ...
    async with async_playwright() as p:
        ...
        # This will also refresh cookies if session is expired
        has_credentials = bool(os.environ.get("ENCUENTRA_EMPLEO_USER") and os.environ.get("ENCUENTRA_EMPLEO_PASS"))
        if has_credentials:
            print("Credentials found - attempting auto-login to refresh session...")
            login_success = await login_and_get_cookies(page)
            if not login_success:
                print("  Auto-login failed, will try with existing cookies...")
        else:
            print("No auto-login credentials - using existing cookies")
        
        # Load cookies from configuration file (fresh or existing)
        ...
        try:
            ...
            if "busquedaOferta.jsf" not in page.url:
                ...
                expired_session = True
                # Try auto-login as last resort if credentials available
                if has_credentials:
                    print("Attempting auto-login as recovery...")
                    login_success = await login_and_get_cookies(page)
                    if login_success:
                        # Reload with fresh cookies
                        await page.goto(URL, timeout=40000)
                        if "busquedaOferta.jsf" not in page.url:
                            print("Still redirected after login recovery. Giving up.")
                            send_notification("Encuentra Empleo - Sesión Expirada", "Auto-login recovery failed.")
                            return
                        print("✅ Session recovered via auto-login!")
                        expired_session = False
                    else:
                        send_notification("Encuentra Empleo - Sesión Expirada", "Auto-login recovery failed. Por favor actualiza config/cookies.json")
                        return
                else:
